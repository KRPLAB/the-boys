...
{
    int id;                                 /* ID do heroi. */
    struct conjunto *habilidades_heroi;     /* conjunto de habilidades que o heroi possui. */
...
struct missao
{
    ...
    struct conjunto *habs_necessarias;      /* habilidades requeridas para concluir uma missao. */
...
/* Libera a memória alocada para o mundo, incluindo herois, bases, missoes e conjuntos de habilidades. Retorna NULL. */
struct mundo *destroi_mundo(struct mundo *m);

/* Inicializa o mundo com heróis, bases e missões iniciais, e adiciona eventos correspondentes à chegada de heróis e início de missões na linha do tempo. */
